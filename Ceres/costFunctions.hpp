...
// Define the regularization term in the cost function. This term encou`rages the 3D points
...
struct LambdaReprojectionError{
	...
	bool operator() (const T* const rot_, const T* const l, T* residuals) const {
		...
		for(int i=0 ; i<42 ; i++) {
			P_[0] += T(l[i])*T(v_[3*i+0]);
			P_[1] += T(l[i])*T(v_[3*i+1]);
			P_[2] += T(l[i])*T(v_[3*i+2]);
		}
...
struct LambdaRegularizer{
	...
	bool operator() (const T* l, T* residuals) const{
		...
		residuals[0] = residuals[1] = residuals[2] = T(0.00);
		for (int i=0;i<42;i++) {
			residuals[0] += l[i]*T(v_[3*i]);
			residuals[1] += l[i]*T(v_[3*i+1]);
			residuals[2] += l[i]*T(v_[3*i+2]);
		}
...
// Cost function to regularize the translation values for a frame with respect to the same for the previouus frame.
struct MultiViewTranslationRegularizer{
	// Constructor
	MultiViewTranslationRegularizer(double *prev_trans) : prev_trans(prev_trans) {}

	// Operator method. Evaluates the cost function and computes the Jacobians.
	template <typename T>
	bool operator() (const T* trans, T* residuals) const{
		residuals[0] = trans[0] - prev_trans[0];
		residuals[1] = trans[1] - prev_trans[1];
		residuals[2] = trans[2] - prev_trans[2];
	}

	// Previous frame translation information.
	double *prev_trans;
};
...
struct PnPError{
	...
	bool operator() (const T* const rot, const T* trans, T* residuals) const {
		...
		P_[2] = T(X_[2]);
		for(int i=0 ; i < 42 ; i++) {
			P_[0] += T(l_[i])*T(v_[3*i]);
			P_[1] += T(l_[i])*T(v_[3*i+1]);
			P_[2] += T(l_[i])*T(v_[3*i+2]);
		}
...
// From IROS '18 paper : Gokul B. Nair

// Ground Plane Estimation error
// The vehicles in the scene must be placed on the respective ground plane
struct GroundPlaneEstimation{
	GroundPlaneEstimation(double *nv, double *d, double *tc)
		: nv(nv), d(d), tc(tc) {}
	template <typename T>
	bool operator()(const T* const rot, const T* trans, T* residuals) const{
		T temp_[3];
		...
		temp_[1] += trans[1];
		temp_[2] += trans[2];
		residuals[0] = T(nv[0])*temp_[0] + T(nv[1])*temp_[1] + T(nv[2])*temp_[2] - T(d[0]);
		return true;
	}
...
// In the scene, the cars' normal must be parallel to the road plane's normal
struct NormalAlignment{
	NormalAlignment(double *ng)
		: ng(ng) {}
	template<typename T>
	bool operator()(const T* nv, T* residuals) const{
		residuals[0] = T(ng[2])*nv[1] - T(ng[1])*nv[2];
		residuals[1] = T(ng[2])*nv[0] - T(ng[0])*nv[2];
		residuals[2] = T(ng[1])*nv[0] - T(ng[0])*nv[1];
		return true;
	}
...
// Normal Alignment Error is minimized when the normals are anti parallel as well

// Base Point Priors
// All base keypoints of the car must lie as close to the ground as possible
struct BasePointPrior{
	BasePointPrior(double *nv, double *trans)
		: nv(nv), trans(trans) {}
	template <typename T>	
	bool operator()(const T* X_base, T* residuals) const{
		residuals[0] = (T(nv[0])*T(trans[0]) + T(nv[1])*T(trans[1]) + T(nv[2])*T(trans[2])) - (T(nv[0])*X_base[0] + T(nv[1])*X_base[1] + T(nv[2])*X_base[2]);
	}
