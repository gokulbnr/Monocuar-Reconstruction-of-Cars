...
int main(int argc, char** argv){
	...
	for(int i = 0; i < numPts; ++i){
		...
		X_bar_fin[3*i] = temp[0];
		X_bar_fin[3*i+1] = temp[1];
		X_bar_fin[3*i+2] = temp[2];
	}

	double *vec1 = new double[3];
	double *vec2 = new double[3];
	for(int i=0;i<3;i++) {
		vec1[i] = X_bar_fin[3*19+i] - X_bar_fin[3*18+i];
		vec2[i] = X_bar_fin[3*0+i] - X_bar_fin[3*18+i];
	}

	double *nv = new double[3];
	nv[0] = vec1[1]*vec2[2] - vec1[2]*vec2[1];
	nv[1] = vec1[2]*vec2[0] - vec1[0]*vec2[2];
	nv[2] = vec1[0]*vec2[1] - vec1[1]*vec2[0];

	double mag = sqrt(nv[0]*nv[0] + nv[1]*nv[1] + nv[2]*nv[2]);
	nv[0] /= mag;
	nv[1] /= mag;
	nv[2] /= mag;
	...
	par[0] = (X_bar_fin[3*0+0] + X_bar_fin[3*1+0] + X_bar_fin[3*18+0] + X_bar_fin[3*19+0])/4 ;
	par[1] = (X_bar_fin[3*0+1] + X_bar_fin[3*1+1] + X_bar_fin[3*18+1] + X_bar_fin[3*19+1])/4 ;
	par[2] = (X_bar_fin[3*0+2] + X_bar_fin[3*1+2] + X_bar_fin[3*18+2] + X_bar_fin[3*19+2])/4 ;

	// Ground Plane Estimation 
	// ceres::CostFunction *groundPlaneEstimation = new ceres::AutoDiffCostFunction<GroundPlaneEstimation, 1, 3>(
	// 	new GroundPlaneEstimation(nv,d));
	// problem.AddResidualBlock(groundPlaneEstimation, new ceres::HuberLoss(0.1), par);
